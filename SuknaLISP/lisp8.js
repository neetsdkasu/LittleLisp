...
//公開先URL
//  http://www.geocities.jp/takeiteasy_idontthinkso/mysoft/js_minimal_lisp/lisp8.html
//本ファイル
//  http://www.geocities.jp/takeiteasy_idontthinkso/mysoft/js_minimal_lisp/lisp8.js
///////////////////////////////////////////////////////////////////////////////////////
//参考文献
//  [1] LISP - Wikipedia
...
//  2013/06/26      Version 1.3.0
//                  Lambda関数の引数の仕様を少し変更した(Schemeライクに)
//  2013/06/27      Version 1.3.1
...
var LISP = new function SukunaLISP () {
    ...
    var executeScriptTags = function () {
        ...
        for (i = 0; i< tags.length; i++) {
            tag = tags[i];
            if ((tag.type == "text/x-sukuna-lisp") || (tag.language == "sukuna-lisp")) {
                src = tag.innerHTML;
    ...
    var ListProcessor = new function () {
        ...
        var executeLisp = function (listLD) {
            if (listLD instanceof LispData) {
                if (listLD.isSymbol()) {
                    ...
                    if (cmdLD.isSystemFunction()) {
                        var func = cmdLD.getData();
                        return func(cmdLD, ListUtil.cdr(listLD));
                    } else if (cmdLD.isUserFunction()) {
                        //ユーザ関数
        ...
        var doLambdaFunction = function (cmdLD, listLD) {
            ...
            VariableManager.jumptoScope(cmdLD.getLambdaVCID());
            VariableManager.intoNewNest();
            if (setLambdaArgs(cmdLD.getLambdaArgDef(), argsLD)) {
                var resultLD = executeLisp(cmdLD.getLambdaProgram());
                VariableManager.outNowNest();
                VariableManager.returnScope();
                return resultLD;
            } else {
                VariableManager.outNowNest();
                VariableManager.returnScope();
                return LispDataManager.makeError("引数エラー！ [" + (cmdLD.getName() != null ? cmdLD.getName() : SPECIALFORM_LAMBDA ) + "]", 
                    "引数の指定がおかしいです！ >> "
                ...
                if (ListUtil.listLength(listLD) > 0) {
                    ...
                    if (ListUtil.listLength(targetLD) == 2) {
                        ...
                        if (testLD.isError()) {
                            ...
                            //次のリストへ
                            var func = cmdLD.getData(); //cmdがisSystemFunction()か未検査
                            return func(cmdLD, ListUtil.cdr(listLD));
                        } else {
                            //対の値を返す
        ...
        SpecialForms[SPECIALFORM_LAMBDA] = new LispData(SPECIALFORM_LAMBDA, function (cmdLD, listLD) {
                if (ListUtil.listLength(listLD) == 2) {
                    ...
                    if (codeLD.isSymbol() || (ListUtil.listLength(codeLD) > 0)) {
                        if (checkLambdaArgDef(argdefLD)) {
                            return LispDataManager.makeLambda(argdefLD, codeLD);
                        } else {
